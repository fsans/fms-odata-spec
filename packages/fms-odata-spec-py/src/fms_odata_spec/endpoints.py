...
FM_ENDPOINTS: List[FMEndpoint] = [
    ...
    FMEndpoint(id="createIndex", method="POST",
               path="/{database}/FileMaker_Indexes/{tableName}",
               description="Create a field index (body: { indexName: <fieldName> })",
               min_version="20",
               category="schema", content_type="application/json"),
    FMEndpoint(id="deleteIndex", method="DELETE",
               path="/{database}/FileMaker_Indexes/{tableName}/{fieldName}",
    ...
    FMEndpoint(id="deleteWebhook", method="POST",
               path="/{database}/Webhook.Delete({webhookId})",
               description="Delete a webhook by id", min_version="22",
               category="webhooks"),
    FMEndpoint(id="getWebhook", method="GET",
               path="/{database}/Webhook.Get({webhookId})",
               description="Get specified webhook data by id", min_version="22",
               category="webhooks"),
    FMEndpoint(id="getAllWebhooks", method="GET",
               path="/{database}/Webhook.GetAll",
               description="Get all webhooks", min_version="22",
               category="webhooks"),
    FMEndpoint(id="invokeWebhook", method="POST",
               path="/{database}/Webhook.Invoke({webhookId})",
               description="Manually invoke a webhook (body: { rowIDs: [...] })",
               min_version="22",
               category="webhooks", content_type="application/json"),
]
