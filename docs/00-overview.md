...
It is not a tutorial, not a wrapper library, and not a runnable application. It is a contract that downstream libraries (MCP servers, JavaScript wrappers, Python clients, PHP clients, etc.) conform to and evolve against.

## Why it exists

The ecosystem has grown from two initial wrapper projects to four downstream implementations:
...
- **Observed behavior**: fms-odata-mcp, fms-odata-js, fms-odata-py, and fms-odata-webhooks repositories (see docs/14-reconciliation.md).
